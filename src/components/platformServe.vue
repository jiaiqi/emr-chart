<template>
  <div class="content-view">
          ...
          <li v-for="(item, index) in SerEsources" :key="index">
            ...
            <dv-border-box-9 style="width: 100%; padding: 0.4rem">
              <div class="bot-card-item bot-card-item-header">
                <span>{{
                  "IP : " +
                  (item.ip == "" || item.ip == undefined ? "暂无数据" : item.ip)
                }}</span>
              ...
              <div class="bot-card-content">
                ...
                <div class="bot-card-content-right">
                    ...
                    <span class="val-text">
                      {{
                        (item.memorySize == "" || item.memorySize == undefined
                          ...
                          : item.memorySize) +
                        ("(" +
                          (item.memory == "" || item.memory == undefined
                            ? diskmemoryDefa
                            : item.memory) +
                          ")")
                      }}
                  ...
                  <div class="bot-card-item">
                    ...
                    <span class="val-text">
                      {{
                        (item.diskSize == "" || item.diskSize == undefined
                          ? dsikDefaules + "G"
                          : item.diskSize) +
                        ("(" +
                          (item.disk == "" || item.disk == undefined
                            ? diskMemoryNumDefa
                            : item.disk) +
                          ")")
                      }}
    ...
    <div v-else>
      <div class="contentif">
        <div class="box-content">暂无服务器资源！</div>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    RunTimeOut () {
      let self = this;
      self.ReqTimeOut.RunTimeOut = new this.timeOut(300, 0, self.petition);
    ...
    diskMemoryData () {
      ...
      let num = 100 - ((this.dsikDefaules / 128) * 100).toFixed(1);
      this.diskMemoryNumDefa = num + "%";
    },
  },
