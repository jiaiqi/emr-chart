...
Vue.prototype.secondToTime = function (val) {
  ...
  if (d < 1) {
    ...
    time = (Number(d) + Number(h / 24)).toFixed(1) + '天' // 若天数大于一且不是一个整数，则输出n.x天
  }
  return time
}
 ...
 * @returns {string}
 */
Vue.prototype.capacity = function (num) {
  let volume = Number(num) * 1024
  if (volume / 1024 / 1024 >= 0.1) {
    if (volume % (1024 * 1024) === 0) {
      volume = (volume / 1024 / 1024) + 'G'
    } else {
      volume = (volume / 1024 / 1024).toFixed(1) + 'G'
    }
  } else if (volume / 1024 >= 0.1) {
    if (num % 1024 === 0) {
      volume = (volume / 1024) + 'MB'
    } else {
      volume = (volume / 1024).toFixed(1) + 'MB'
    }
  } else {
    volume = volume.toFixed(1) + "KB"
  }
  return volume
}
