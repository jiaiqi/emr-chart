...
export class vChartInfo {
	constructor() {
		...
		this.colArrData = this.getColumns = () => {
			let arr = []
			arr.push()
		}
		this.dataType = true
				...
				for (let i = 0; i < datasType.length; i++) {
					...
					for (let j = 0; j < timeArrKey.length; j++) {
						...
						for (let n = 0; n < dataMapkey.length; n++) {
							...
							itemData[dataMapkey[n]] = this.getDataCount(
								itemCol,
								datas,
								vChartType,
								countCol
				...
				if (data1 && data1.length > 0) {
					for (let d in data1[0]) {
						...
						if (num > 1 && num <= max) {
							// 原始数据是一个数值，max必须大于1
							num = num / max
						} else if (num <= 1 && (num >= 0) & (max == 1)) {
							// 原始数据是百分比，max必须设置为1即100%
							num = num
						}
		...
		this.getDataCount = (itemCol, data, chartType, countCol) => {
			...
			if (
				...
				for (let i = 0; i < data.length; i++) {
					if (
						(itemCol.dataTypeCol.value !== 'all'
							? data[i][itemCol.dataTypeCol.key] === itemCol.dataTypeCol.value
							: true) &&
						data[i][itemCol.timeArr.key] == itemCol.timeArr.value &&
						data[i][itemCol.dataMap.key] === itemCol.dataMap.value
					)
						count += data[i][countCol]
				}
			} else if (chartType === 'pie' || chartType === 'gauge') {
				// 增加饼图处理 2019年9月27日
				for (let i = 0; i < data.length; i++) {
					if (
						(itemCol.dataTypeCol.value !== 'all'
